...
import { studentIdToStudentDetails } from "../utils/converter.utils";

const prisma = new PrismaClient();

async function sendNotifsToParents(message: string, sender: string, currParentId: string) {
...
const AssignmentController = {
    didStudentSubmitAssignment: async(req: Request, res: Response) => {
    ...
    getGradedAssignments: async(req: Request, res:Response) => {
        const { classCode } = req.body;
        const userId = req.user?.id; 
    ...
    submitAssignment: async(req: Request, res: Response) => {
        ...
        try {
            ...
            for (z in allGradedAssignmentsByStudent) {
                totalGradePoints += allGradedAssignmentsByStudent[z].grade as number
            }
            ...
            const studentName = await studentIdToStudentDetails(userId as string);
